...
-> 공유 기본키 전략의 경우 지연로딩 옵션을 주더라도, 프록시 지연로딩을 사용할 수 없다. 지연로딩으로 설정할 경우 프로퍼티가 널을 허용할 경우 데이터를 쿼리해서 값을 확인하는 작업이 발생함.
...
name: 중간 테이블의 이름을 지정.
joinColumns: 현재 엔티티와 중간 테이블 간의 조인 컬럼을 정의.
inverseJoinColumns: 다른 엔티티와 중간 테이블 간의 조인 컬럼을 정의.
...
```
- 임베드된 컴포넌트의 식별 값은 임베드한 컴포넌트의 식별 값이다.
- 임베드된 컴포넌트는 공유 참조를 가질 수 없음. 따라서 매핑된 ``Shipment``도 참조를 가질 수 없음.
